...
import ru.practicum.EwmStatsClient;
...
import ru.practicum.exception.InvalidDataArgsException;
...
public class EventServiceImpl implements EventService {
    private final EwmStatsClient statsClient  = new EwmStatsClient();
    ...
    public EventDto save(Long userId, NewEventDto newEventDto) {
        if (newEventDto.getEventDate().isBefore(LocalDateTime.now())) {
            throw new InvalidDataArgsException(String.format("Ошибка времени: %s", newEventDto.getEventDate()));
        }
    ...
    public EventFullDto updateForUser(Long userId, Long eventId, UpdateEventUserRequest updateEventUserRequest) {
        if (updateEventUserRequest.getEventDate() != null
            ...
            throw new InvalidDataArgsException(
        ...
        if (eventForUpdate.getState().equals(EventState.PUBLISHED)) {
            throw new ConflictException(String.format("Ошибка статуса: %s", eventForUpdate.getState()));
        }
        ...
        if (updateEventUserRequest.getLocation() != null) {
            ...
            location.setLon(updateEventUserRequest.getLocation().getLongitude());
            location.setLat(updateEventUserRequest.getLocation().getLatitude());
        }
    ...
    private void saveEndpointHit(HttpServletRequest request) {
        EndpointHitDto endpointHitDto = EndpointHitDto.builder()
                ...
                .build();
        statsClient.hitRequest(endpointHitDto);
    }

    private void setView(Event event) {
        int view = statsClient.getStats(event.getCreatedOn(),
                LocalDateTime.now(),
